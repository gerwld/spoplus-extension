...
<style id="43ggfdbt5rf">
  ...
  button.close_btn {
    ...
    border-radius: 100px;
            ...
            `;setTimeout(()=>{document.body.appendChild(a);var e=document.querySelectorAll(".closeNotification"),e=(e&&e?.forEach(e=>e.addEventListener("click",function(){a.style.display="none"})),s.storage.local.set({closeDonateCount:1+(isNaN(n.closeDonateCount)?0:+n.closeDonateCount)}),"string"==typeof t?.formState?.theme&&s.storage.local.set({formState:{...t.formState,timestamp:Date.now()}}),document.querySelectorAll(".donate-btn"));e.length&&e.forEach(e=>e.addEventListener("click",function(){s.storage.local.set({closeDonateCount:7})}))},24e4)}})})},!1)}})();